  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: T.bg }}>
      ...
      <div style={{ padding: isDesktop ? '24px 40px 32px' : '16px 20px 24px' }}>
        ...
        {!loading && listings.length > 0 && (
          <>
            <div style={{ fontSize: 11, fontWeight: 700, color: T.textSub, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>
              Your Listings
            </div>
            <div style={isDesktop ? { display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14 } : { display: 'flex', flexDirection: 'column', gap: 10 }}>
              {listings.map(l => {
                ...
                const upcoming    = isUpcoming(l.date)
                ...
                return (
                  <div
                    ...
                    onMouseEnter={e => {
                      ...
                      e.currentTarget.style.borderColor = isConfirming ? '#E05252' : T.primary
                    }}
                    onMouseLeave={e => {
                      e.currentTarget.style.transform   = 'translateY(0)'
                      e.currentTarget.style.boxShadow   = '0 1px 4px rgba(0,0,0,0.05)'
                      e.currentTarget.style.borderColor = isConfirming ? '#E05252' : T.border
                    }}
                    style={{
                      background: T.card, borderRadius: 14,
                      border: `1px solid ${isConfirming ? '#E05252' : T.border}`,
                  ...
                  >
                    <button
                      onClick={() => onSelectOpp && onSelectOpp({
                        id: `org_${l.id}`, title: l.title, org: user.name,
                        org_id: l.org_id || user.id, cause: l.cause, location: l.location,
                        hours: l.hours, date: l.date, description: l.description,
                        externalUrl: l.external_url, remote: !!l.remote, source: 'org',
                      })}
                    ...
                    >
                      <div style={{ fontSize: 15, fontWeight: 600, color: T.text, marginBottom: 8 }}>{l.title}</div>

                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 10 }}>
                        <span style={{ background: cause.bg, color: cause.text, fontSize: 11, fontWeight: 600, padding: '3px 9px', borderRadius: 20 }}>{l.cause}</span>
                        {l.age_group && l.age_group !== 'all' && (
                          <span style={{ background: '#F0F4FF', color: '#4A6FA5', fontSize: 11, fontWeight: 500, padding: '3px 9px', borderRadius: 20 }}>{l.age_group}</span>
                        )}
                        {l.remote && (
                          <span style={{ background: T.primaryLight, color: T.primary, fontSize: 11, fontWeight: 500, padding: '3px 9px', borderRadius: 20 }}>Remote</span>
                        )}
                        {l.date && (
                          <span style={{
                            background: upcoming ? '#E8F8EF' : '#F4F4F4',
                            color: upcoming ? T.primaryDark : T.textMuted,
